IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'OMS_SKU_COMPOSICAO')
	...
	SELECT	'OMS_SKU_COMPOSICAO'   AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			'1'                    AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			'1'                    AS VALOR_ATUAL,
			'Indica a composicao que sera utilizada no SKU.' AS DESC_PARAMETRO,
			'I'                   AS TIPO_DADO,
			''                    AS RANGE_VALOR_ATUAL,
			0                     AS GLOBAL,
			'1 - Produto + Cor + Tamanho 2 - Produto + Cor + Grade'                 AS NOTA_PROGRAMADOR,
			0                     AS ESCOPO,
			0                     AS POR_USUARIO_OK,
