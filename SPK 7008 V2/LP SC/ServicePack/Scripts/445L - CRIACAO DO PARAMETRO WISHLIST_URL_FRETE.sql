IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'WISHLIST_URL_FRETE')
	...
	SELECT	'WISHLIST_URL_FRETE'   AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			''                 AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			''                 AS VALOR_ATUAL,
			'URL para consulta do valor de Frete Lista de Presente ECOM.' AS DESC_PARAMETRO,
			'C'                   AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0                     AS GLOBAL,
			'URL para consulta do valor de Frete Lista de Presente ECOM.'  AS NOTA_PROGRAMADOR,
			0                     AS ESCOPO,
			0                     AS POR_USUARIO_OK,
