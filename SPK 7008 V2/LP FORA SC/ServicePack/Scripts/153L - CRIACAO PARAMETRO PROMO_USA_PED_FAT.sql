IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'PROMO_USA_PED_FAT')
	...
	SELECT	'PROMO_USA_PED_FAT'   AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			'.F.'                 AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			'.F.'                 AS VALOR_ATUAL,
			'Indica se os itens de pedido sendo faturado na venda devem ser utilizados como gatilho de promoção.' AS DESC_PARAMETRO,
			'L'                   AS TIPO_DADO,
