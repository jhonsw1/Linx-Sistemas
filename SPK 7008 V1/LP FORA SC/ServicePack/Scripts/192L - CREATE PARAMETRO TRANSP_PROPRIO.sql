IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'TRANSP_PROPRIO')
	...
	SELECT	'TRANSP_PROPRIO'      AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			''                    AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			''                    AS VALOR_ATUAL,
			'transportadoras que representam o próprio estabelecimento (separados por "|" (pipe)).' AS DESC_PARAMETRO,
			'C'                   AS TIPO_DADO,
