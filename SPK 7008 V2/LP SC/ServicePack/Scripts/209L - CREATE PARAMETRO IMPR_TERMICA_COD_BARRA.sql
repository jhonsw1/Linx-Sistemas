IF NOT EXISTS(SELECT PARAMETRO FROM PARAMETROS WHERE PARAMETRO = 'IMPR_TERMICA_COD_BARRA')																 
	...
	SELECT	'IMPR_TERMICA_COD_BARRA' AS PARAMETRO,
			CAST(0 AS DATETIME) PENULT_ATUALIZACAO,
			'.F.' AS VALOR_DEFAULT,
			GETDATE() AS ULT_ATUALIZACAO,
			'.F.' AS VALOR_ATUAL,
			'HABILITA TELA PARA IMPRESSÃO TÉRMICA DE CÓDIGOS DE BARRAS.' AS DESC_PARAMETRO,
			'L' AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0 AS GLOBAL,
			'PARÂMETRO PARA EXIBIR A TELA DE IMPRESSÃO TÉRMICA DE CÓDIGOS DE BARRAS EM <MÓDULOS EXCLUSIVOS>.' AS NOTA_PROGRAMADOR,
			0 AS ESCOPO,
			0 AS POR_USUARIO_OK,
