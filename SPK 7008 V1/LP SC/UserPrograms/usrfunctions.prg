*## Hering Userprograms Versao: 20221025
*## TIPO_FILIAL: PROPRIA
*## VERSAO_MIDE: 1.0.59
...
Define Class UserHering As Custom	
	...
	Procedure VerificaSeJaEstaEmexecucao
		...
		IF CountProcessByName("LPMLib.exe") > 1
			ShowProgress("O Aplicativo já está em execução")
			sleep(3000)
			ShowProgress()
			Main.ExitCommand()
			Quit
	...
	Procedure MsgTransitoPendente() As Boolean && Verifica Notas em Transito
		...
		If SQLSelect(strConsultaTransito ,[CurTransito],[Erro ao consultar notas em transito.\n\nVerifique se existem notas em transito.])
			...
			If Reccount("CurTransito")==0
				MsgBox([Erro ao consultar notas em transito.\n\nVerifique se existem notas em transito.],-48,[Atenção])
			Endif
			If CurTransito.numeroDeNotasEmTransito >0
				MsgBox([A loja possui ]+Alltrim(Str(CurTransito.numeroDeNotasEmTransito))+[ notas fiscais a mais de 20 dias pendentes no trânsito. \n\nFavor entrar em contato com a retaguarda.],-48,[Atenção])
				bRetorno = .F.
			Endif
			Use In CurTransito
		Endif
...
Define Class hrInputBox As Form
	...
	Procedure Init
		...
		Thisform.lb_info.Caption = Nvl(Evl(strInfo ,Null),"Entre com a informação necessária")
		Thisform.Caption = Nvl(Evl(strTitulo ,Null),"Informação")
...
ENDFUNC
** FIM - Métodos CRM - Compatilhado

** Métodos CRM - Cliente CRM
...
Procedure ValidarEmailSafetyMail() As Custom
	...
	oRetorno.AddProperty("Mensagem","Não Foi possível validar e-mail")
	...
	Try
		...
		*!* Fim - Busca CNPJ
		...
		strLog = "Email "+Nvl(strEmail,"{Null}")+" - válido: "+Iif(oRetorno.EmailValido,"Sim","Não")+" - "+Nvl(oRetorno.Mensagem,"")
...
PROCEDURE ConsultarClienteCrm()	
	...
	ENDTRY	
ENDPROC
** Métodos CRM - Fim Cliente CRM

** Métodos Novos  CRM - Cliente CRM
...
FUNCTION ConsultarTelefoneClienteCrm()	
	...
	RETURN objCrmCliente 	
ENDPROC
** Métodos Novos CRM - Fim Cliente CRM


** Métodos Novos MOTOR - Voucher CRM
...
** Métodos Novos MOTOR - Fim Voucher CRM


** Métodos CRM - Voucher CRM
...
Function AtivarVoucher()	
	...
	CATCH TO oError 
		strMessageErro = "Erro no procedimento de validação do voucher. Erro: "+oError.Message
...
** Métodos CRM - Fim Voucher CRM

** Procedure para identificar filiais próprias
...
Procedure HrCriaPropriedadeFilialPropria
	...
	IF RECCOUNT("curHrTipoFilial") ==0 
		...
		MSGBOX("ERRO AO BUSCAR TIPO DA FILIAL AVISE O SUPORTE",0,"ATENÇÃO")
		return.f.
