IF NOT EXISTS(SELECT PARAMETRO FROM PARAMETROS WHERE PARAMETRO = 'RELAT_ENDEREC_ESTOQUE')
	...
	SELECT	'RELAT_ENDEREC_ESTOQUE' AS PARAMETRO,
			CAST(0 AS DATETIME) PENULT_ATUALIZACAO,
			'.F.' AS VALOR_DEFAULT,
			GETDATE() AS ULT_ATUALIZACAO,
			'.F.' AS VALOR_ATUAL,
			'HABILITA BOTÃO PARA IMPRIMIR RELATÓRIO DE ENDEREÇAMENTO DE ESTOQUE.' AS DESC_PARAMETRO,
			'L' AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0 AS GLOBAL,
			'HABILITAR BOTÃO PARA IMPRIMIR RELATÓRIO DE ENDEREÇAMENTO DE ESTOQUE NA TELA <CONSULTA DE VENDAS POR PRODUTO> EM <CONSULTAS GERENCIAIS>.' AS NOTA_PROGRAMADOR,
			0 AS ESCOPO,
			0 AS POR_USUARIO_OK,
