...
IF NOT EXISTS(SELECT PARAMETRO FROM PARAMETROS WHERE PARAMETRO = 'OMS_OPERACAO_ENTRADA')
	...
	SELECT	'OMS_OPERACAO_ENTRADA' AS PARAMETRO,
			CAST(0 AS DATETIME) PENULT_ATUALIZACAO,
			'.F.' AS VALOR_DEFAULT,
			GETDATE() AS ULT_ATUALIZACAO,
			'.F.' AS VALOR_ATUAL,
			'HABILITA A OPERACAO DE ENTRADA NA EMISSAO DE NOTA EM TROCA ORIGINARIA OMS.' AS DESC_PARAMETRO,
			'L' AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0 AS GLOBAL,
			'QUANDO HABILITADO, OPERACAO DE ENTRADA SERA HABILITADA APENAS EM TROCAS ONDE O FATURAMENTO SEJA EM FILIAL DIFERENTE DA TROCA.' AS NOTA_PROGRAMADOR,
			0 AS ESCOPO,
			0 AS POR_USUARIO_OK,
