IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'WISHLIST_USUARIO')
	...
	SELECT	'WISHLIST_USUARIO'   AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			''                 AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			''                 AS VALOR_ATUAL,
			'Usuario para autenticação nas APIS do LinxComerce.' AS DESC_PARAMETRO,
			'C'                   AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0                     AS GLOBAL,
			'Usuario para autenticação nas APIS do LinxComerce.'  AS NOTA_PROGRAMADOR,
			0                     AS ESCOPO,
			0                     AS POR_USUARIO_OK,
