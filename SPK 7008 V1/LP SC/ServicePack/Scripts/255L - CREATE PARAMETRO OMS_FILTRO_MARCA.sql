...
IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'OMS_FILTRO_MARCA')
	...
	SELECT	'OMS_FILTRO_MARCA' AS PARAMETRO,
			CAST(0 AS DATETIME) PENULT_ATUALIZACAO,
			'' AS VALOR_DEFAULT,
			GETDATE() AS ULT_ATUALIZACAO,
			'' AS VALOR_ATUAL,
			'Marca que será utilizada como filtro no terminal.' AS DESC_PARAMETRO,
			'C' AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0 AS GLOBAL,'É possível incluir diversas Marcas por terminal separando por ; cada marca.' AS NOTA_PROGRAMADOR,
			0 AS ESCOPO,
			0 AS POR_USUARIO_OK,
