IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'SERV_HUB_PARTNER')
	...
	SELECT	'SERV_HUB_PARTNER'   AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			''                 AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			''                 AS VALOR_ATUAL,
			'Indica se utiliza o Service HUB ou OMS.' AS DESC_PARAMETRO,
			'C'                   AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0                     AS GLOBAL,
			'Indica se utiliza o Service HUB ou OMS.'  AS NOTA_PROGRAMADOR,
			0                     AS ESCOPO,
			0                     AS POR_USUARIO_OK,
