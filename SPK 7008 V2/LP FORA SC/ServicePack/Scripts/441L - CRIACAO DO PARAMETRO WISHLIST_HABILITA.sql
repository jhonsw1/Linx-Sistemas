IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'WISHLIST_HABILITA')
	...
	SELECT	'WISHLIST_HABILITA'   AS PARAMETRO,
			CAST(0 AS DATETIME)   AS PENULT_ATUALIZACAO,
			'.F.'                 AS VALOR_DEFAULT,
			GETDATE()             AS ULT_ATUALIZACAO,
			'.F.'                 AS VALOR_ATUAL,
			'Indica se utiliza a Lista de Presente ECOM.' AS DESC_PARAMETRO,
			'L'                   AS TIPO_DADO,
			''					  AS RANGE_VALOR_ATUAL,
			0                     AS GLOBAL,
			'Indica se utiliza a Lista de Presente ECOM.' AS NOTA_PROGRAMADOR,
			0                     AS ESCOPO,
			0                     AS POR_USUARIO_OK,
