...
IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'OMS_TROCA_OBRIGA_SAC')
	...
	SELECT	'OMS_TROCA_OBRIGA_SAC' AS PARAMETRO,
			CAST(0 AS DATETIME) PENULT_ATUALIZACAO,
			'.F.' AS VALOR_DEFAULT,
			GETDATE() AS ULT_ATUALIZACAO,
			'.F.' AS VALOR_ATUAL,
			'Trocas e devoluções relacionadas ao OMS devem obrigatoriamente passar pelo SAC.' AS DESC_PARAMETRO,
			'L' AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0 AS GLOBAL,'Trocas e devoluções relacionadas ao OMS devem obrigatoriamente passar pelo SAC.' AS NOTA_PROGRAMADOR,
			0 AS ESCOPO,
			0 AS POR_USUARIO_OK,
