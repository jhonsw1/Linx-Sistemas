IF NOT EXISTS(SELECT TOP 1 1 FROM PARAMETROS WHERE PARAMETRO = 'ENTRADA_CEGA')
	...
	SELECT	'ENTRADA_CEGA' AS PARAMETRO,
			CAST(0 AS DATETIME) PENULT_ATUALIZACAO,
			'.F.' AS VALOR_DEFAULT,
			GETDATE() AS ULT_ATUALIZACAO,
			'.F.' AS VALOR_ATUAL,
			'Não exibir na tela a relação dos produtos quando for realizado uma entrada por pedido.' AS DESC_PARAMETRO,
			'L' AS TIPO_DADO,
			'' AS RANGE_VALOR_ATUAL,
			0 AS GLOBAL,
			'Utilizado na tela de Entrada de Produtos do Linx POS Manager, para realizar uma entrada por pedido sem exibir os produtos no grid.' AS NOTA_PROGRAMADOR,
			0 AS ESCOPO,
			0 AS POR_USUARIO_OK,
			0 AS PERMITE_POR_EMPRESA,
			1 AS ENVIA_PARA_LOJA,
			0 AS PERMITE_POR_LOJA,
